...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == ADMIN_ID:
        await update.message.reply_text("👋 أهلاً بك يا Batman! اختر زر انقاذ المدينـه😂🦾:", reply_markup=main_menu())
    else:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("insta_"):
        ...
        await query.edit_message_text(f"📸 إدارة حساب باتمغرام: {account}\n- يمكنك تغيير المعلومات أو التحكم بالتفاعل.", reply_markup=main_menu())
    elif data.startswith("tg_"):
        account = data.replace("tg_", "")
        await query.edit_message_text(f"💬 إدارة حساب باتمجرام: {account}\n- يمكنك تغيير المعلومات أو التحكم بالتفاعل.", reply_markup=main_menu())
    elif data == "settings":
